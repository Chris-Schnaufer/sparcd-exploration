...
**Draft**: An local edit held in the browser until a Sync writes it
...
**Observation**: A single species identification recorded against image — a
species, a count, and any notes. In the data model the `observations.csv`
file holds one row per Observation.

**Snapshot**: An unchangable backup of a Collection/Upload's saved files,
taken just before a Sync, so a Sync can be undone.

**Species**: A predefine scientific name paired with a common name. May also
used to refer to the list of these name pairs (although using "species list"
is more common).

**Storage endpoint**: The Endpoint URL specied at login time. This is an
...
as part of a collection. Consists of the image files, location, specied, and
